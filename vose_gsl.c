...
int* vose(gsl_vector *v, int length, int n);
void prepare_vose(gsl_vector *v, int length, int *alias, double *prob);
...
int* vose(gsl_vector *v, int length, int n){
	...
	prepare_vose(v, length, alias, prob); //prepares the arrays, complexity O(length);
...
void prepare_vose(gsl_vector *v, int length, int *alias, double *prob) {
	...
	for(;i<length;i++){
		probability = gsl_vector_get (v, i);
		probability *= length;
		gsl_vector_set(v,i,probability); 
	...
	while(small!=NULL && large!=NULL){
		...
		prob[l] = gsl_vector_get(v,l);
		alias[l] = g;

		// probabilities[g]+=probabilities[l]-1;
		prob_tmp = gsl_vector_get(v,g) + gsl_vector_get(v,l)-1;
		gsl_vector_set(v,g,prob_tmp);
		// if(probabilities[g] < 1) append(g, &small);
		if(gsl_vector_get(v,g) < 1) append(g, &small);
		else append(g, &large);
	}
